...
def prediction(request: PredictRequest):
    ...
    log.info(event="prediction",request_id = str(request_id), input = request.features, output = int(predicted_value), model_version = "v1", latency_ms = latency)
    return {"prediction": int(predicted_value), "probability": float(probability),
            "model_version": "v1", "request_id": str(request_id)}